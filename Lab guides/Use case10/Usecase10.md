...
Azure OpenAI
在数据上的主要功能之一是它能够以增强模型输出的方式检索和利用数据。針對資料的
Azure OpenAI 與 Azure
認知搜索一起，根據使用者輸入和提供的對話歷史記錄確定要從指定資料來源檢索的資料。然後，這些資料被擴充並作為提示重新提交到
OpenAI
模型，檢索到的資訊將附加到原始提示中。儘管檢索到的資料被附加到提示中，但生成的輸入仍由模型處理，就像任何其他提示一樣。檢索到資料並將提示提交到模型後，模型將使用此資訊提供完成。

**目标**

- 在 Azure 門戶中創建存儲帳戶、容器和 Azure 認知搜索服務。
...
- 啟動 Copilot 並開始與機器人對話

- 啟動新應用程式並開始與 Copilot 應用程式對話。

- 刪除 gpt-3-turbo 和嵌入式模型、Azure 存儲帳戶、認知搜索服務和新的 Web
  應用。

## 練習 1 - 使用門戶創建 Azure 存儲帳戶和 Azure 認知搜索
...
10. 在 **Network** 選項卡中，將所有選項按鈕保留為預設狀態，然後按一下
    **Next** 按鈕。

![](./media/image9.png)

11. 在 **Tags** 選項卡中，將所有欄位保留為預設狀態，然後按一下 **Next**
    按鈕。
...
### 任務 2：使用門戶創建 Azure 存儲帳戶

1.  登錄 +++<https://portal.azure.com/+++>
...
8.  這個新的 Azure 存儲帳戶現已設置為託管 Azure 資料湖的資料。单击 **Go
    to resource** 按钮。
...
8.  等待 10 分鐘，以便可以在應用程式上成功應用身份驗證配置。

9.  10 分鐘後，按一下 **Refresh** 按鈕。

10. 在 **Permissions requested** 对话框中，单击 **Accept** 按钮

![](./media/image71.png)

11. 現在，Web 應用程式將在新流覽器中打開。

![](./media/image72.png)

12. 在 **Azure AI** Web 應用程式頁面中，輸入以下文本，然後按一下
    **Submit 圖示**，如下圖所示。
...
13. 同樣，將以下文本粘貼到文字方塊中，然後按一下 **Send** 圖示。
...
4.  從左側窗格中選擇 **Agents**。然后点击 **+ New agent**。

> ![](./media/image90.png)

5.  選擇 **Skip to configure**。
...
10. 這將添加 Azure OpenAI 服務連接並打開連接屬性窗格。
...
3.  你將收到來自 **Azure OpenAI
    資源**中上傳的資料的響應。此外，请注意回复下方的 **Surfaced with
    Azure OpenAI** 消息。
...
设置。然後，您啟動了一個新應用程式並開始與聊天機器人對話。你已删除
